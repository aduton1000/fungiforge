...
include { BASECALL } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage00_basecall.nf'
...
workflow {
    ...
    //run process
    BASECALL.run(input.toArray())

    if (BASECALL.output){

        // consumes all named output channels and stores items in a json file
        BASECALL.out.getNames().each { name ->
            serializeChannel(name, BASECALL.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = BASECALL.out as List<Object>
        def i = 0
        array.each { output ->
