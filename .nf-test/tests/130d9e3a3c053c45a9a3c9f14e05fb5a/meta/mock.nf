...
include { ASSEMBLE } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage02_assemble.nf'
...
workflow {
    ...
    //run process
    ASSEMBLE.run(input.toArray())

    if (ASSEMBLE.output){

        // consumes all named output channels and stores items in a json file
        ASSEMBLE.out.getNames().each { name ->
            serializeChannel(name, ASSEMBLE.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = ASSEMBLE.out as List<Object>
        def i = 0
        array.each { output ->
