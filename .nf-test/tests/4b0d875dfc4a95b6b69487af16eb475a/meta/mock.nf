...
include { REPEATMASK } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage06_repeatmask.nf'
...
workflow {
    ...
    //run process
    REPEATMASK.run(input.toArray())

    if (REPEATMASK.output){

        // consumes all named output channels and stores items in a json file
        REPEATMASK.out.getNames().each { name ->
            serializeChannel(name, REPEATMASK.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = REPEATMASK.out as List<Object>
        def i = 0
        array.each { output ->
