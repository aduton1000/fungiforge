...
include { NOVELTY } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage12_novelty.nf'
...
workflow {
    ...
    //run process
    NOVELTY.run(input.toArray())

    if (NOVELTY.output){

        // consumes all named output channels and stores items in a json file
        NOVELTY.out.getNames().each { name ->
            serializeChannel(name, NOVELTY.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = NOVELTY.out as List<Object>
        def i = 0
        array.each { output ->
