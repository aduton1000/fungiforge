...
include { REPORT } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage14_report.nf'
...
workflow {
    ...
    //run process
    REPORT.run(input.toArray())

    if (REPORT.output){

        // consumes all named output channels and stores items in a json file
        REPORT.out.getNames().each { name ->
            serializeChannel(name, REPORT.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = REPORT.out as List<Object>
        def i = 0
        array.each { output ->
