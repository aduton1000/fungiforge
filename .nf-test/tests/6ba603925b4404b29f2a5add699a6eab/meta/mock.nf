...
include { IDENTIFY } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage08_identify.nf'
...
workflow {
    ...
    //run process
    IDENTIFY.run(input.toArray())

    if (IDENTIFY.output){

        // consumes all named output channels and stores items in a json file
        IDENTIFY.out.getNames().each { name ->
            serializeChannel(name, IDENTIFY.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = IDENTIFY.out as List<Object>
        def i = 0
        array.each { output ->
