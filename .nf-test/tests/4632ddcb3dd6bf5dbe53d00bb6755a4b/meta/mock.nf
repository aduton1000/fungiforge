...
include { ANNOTATE } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage07_annotate.nf'
...
workflow {
    ...
    //run process
    ANNOTATE.run(input.toArray())

    if (ANNOTATE.output){

        // consumes all named output channels and stores items in a json file
        ANNOTATE.out.getNames().each { name ->
            serializeChannel(name, ANNOTATE.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = ANNOTATE.out as List<Object>
        def i = 0
        array.each { output ->
