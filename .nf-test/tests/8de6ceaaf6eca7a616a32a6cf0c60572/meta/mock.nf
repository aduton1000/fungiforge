...
include { READ_QC } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage01_readqc.nf'
...
workflow {
    ...
    //run process
    READ_QC.run(input.toArray())

    if (READ_QC.output){

        // consumes all named output channels and stores items in a json file
        READ_QC.out.getNames().each { name ->
            serializeChannel(name, READ_QC.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = READ_QC.out as List<Object>
        def i = 0
        array.each { output ->
