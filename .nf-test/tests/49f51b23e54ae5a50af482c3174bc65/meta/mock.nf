...
include { DB_CHECK } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage00_dbcheck.nf'
...
workflow {
    ...
    //run process
    DB_CHECK.run(input.toArray())

    if (DB_CHECK.output){

        // consumes all named output channels and stores items in a json file
        DB_CHECK.out.getNames().each { name ->
            serializeChannel(name, DB_CHECK.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = DB_CHECK.out as List<Object>
        def i = 0
        array.each { output ->
