...
include { SRPOLISH } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage03b_srpolish.nf'
...
workflow {
    ...
    //run process
    SRPOLISH.run(input.toArray())

    if (SRPOLISH.output){

        // consumes all named output channels and stores items in a json file
        SRPOLISH.out.getNames().each { name ->
            serializeChannel(name, SRPOLISH.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = SRPOLISH.out as List<Object>
        def i = 0
        array.each { output ->
