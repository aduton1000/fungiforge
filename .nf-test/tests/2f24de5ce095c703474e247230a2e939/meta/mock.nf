...
include { ASSEMBLY_QC } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage05_assembly_qc.nf'
...
workflow {
    ...
    input = []
                ...
                input[0] = [[id:'S1', assembly_mode:'longread', compartment:'AIR', facility:'F1', season:'Dry'], file("/Users/brightadu/Documents/Claude/Projects/fungiforge/test/samplesheet.test.csv")]
                
    //----

    //run process
    ASSEMBLY_QC.run(input.toArray())

    if (ASSEMBLY_QC.output){

        // consumes all named output channels and stores items in a json file
        ASSEMBLY_QC.out.getNames().each { name ->
            serializeChannel(name, ASSEMBLY_QC.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = ASSEMBLY_QC.out as List<Object>
        def i = 0
        array.each { output ->
