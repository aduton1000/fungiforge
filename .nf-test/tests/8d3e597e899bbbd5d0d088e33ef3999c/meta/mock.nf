...
include { BGC } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage11_bgc.nf'
...
workflow {
    ...
    //run process
    BGC.run(input.toArray())

    if (BGC.output){

        // consumes all named output channels and stores items in a json file
        BGC.out.getNames().each { name ->
            serializeChannel(name, BGC.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = BGC.out as List<Object>
        def i = 0
        array.each { output ->
