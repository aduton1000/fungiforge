...
include { DECONTAM } from '/Users/brightadu/Documents/Claude/Projects/fungiforge/modules/stage04_decontam.nf'
...
workflow {
    ...
    //run process
    DECONTAM.run(input.toArray())

    if (DECONTAM.output){

        // consumes all named output channels and stores items in a json file
        DECONTAM.out.getNames().each { name ->
            serializeChannel(name, DECONTAM.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = DECONTAM.out as List<Object>
        def i = 0
        array.each { output ->
