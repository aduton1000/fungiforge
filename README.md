...
- **Non-fungal isolates are stopped early:** after decontamination and assembly QC, an isolate whose contigs are bacterial or human, or whose assembly fails QC, keeps its master row (with the reason) but is not annotated; `--force_all` overrides.
